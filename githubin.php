...
//obtenemos el contenido
 ...
 //limpiamos el header
	 ...
	 {
		case 'readme':
			$content=preg_replace('/<!DOCTYPE[\s\S]*?<article/imu','<article',$content);
		 break;
		 case 'folder':
			$content=preg_replace('/<!DOCTYPE[\s\S]*?<div class=\"list files-list\"/imu','<div class="list files-list"',$content);
			$content=preg_replace('/<\/div>[\s]*<footer class=/imu','<footer class',$content);	//quitamos el último </div> ya que empezamos desde el segundo div no desde el primero.
		 break;
		 case 'file':
			$content=preg_replace('/<!DOCTYPE[\s\S]*?<pre>/imu','<div id="github_file">',$content);
			$content=preg_replace('/<\/pre>[\s\S]*<footer class/imu','</div> <footer class',$content);
		 break;
		 case 'repos':
			$content=preg_replace('/<!DOCTYPE[\s\S]*?<div class=\"list repo-list/imu','<div class="list repo-list',$content);
		 break;
	 }
	 
 //limpiamos el footer
	$content=preg_replace('/<footer class[\s\S]*<\/html>/imu','',$content);
	
	
 //quitamos las imágenes (si así se ha querido)
	if( isset($atts["disable_images"]) && $atts["disable_images"]!='false' )
		$content=preg_replace('/<img [\s\S]*?>/imu','',$content);

 //quitamos fav del repositorio
	if( isset($atts["disable_fav"]) && $atts["disable_fav"]!='false')
		$content=preg_replace('/\<p class=\"text\-gray text\-small mb\-0 mt\-2[\s\S]*?<\/p>/imu','',$content);
		
 //quitamos los css	(ya las hemos quitado en el header)
	//$content=preg_replace('/<link .+?\>/imu','',$content);
